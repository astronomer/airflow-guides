...
The preferred methods for retrieving data from the metadata database are using the Airflow UI or making a request to the Airflow API. The UI and API provide viewing access to much of the data in the metadata database, and accessing data using them reduces the potential of accidentally making changes to data that can harm your Airflow instance. For use cases where neither the Airflow UI nor the REST API can provide sufficient data it is recommended to use SQLAlchemy to query the metadata database.
