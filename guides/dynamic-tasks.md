...
t2 = BashOperator.partial(task_id="t2").expand_kwargs(kwargs)
```
...
def add_numbers(x,y,z):
    # the op_args keyword of the PythonOperator performs the unpacking of the
...
Both tasks, `TaskFlow_add_numbers` and `standard_add_numbers` will have three mapped task instances each performing one calculation with the results being:
