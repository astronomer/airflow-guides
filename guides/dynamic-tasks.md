...
Tasks have two new functions available to implement this feature:

- `expand()`: This function passes a group of inputs that maps to a single parameter for a given operator. It then creates mapped task runs for each input value. 
- `partial()`: This function passes a group of inputs that apply to all mapped task runs which are generated by `expand()`.

The following task uses both of these functions to dynamically generate 3 task runs:
...
- You *can't* map over specific parameter types. For example, `task_id`, `pool`, and many `BaseOperator` arguments are not mappable.
