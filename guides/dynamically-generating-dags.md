...
- Since this method requires a Python file in the `DAG_FOLDER`, the generation code will be executed on every Scheduler heartbeat. This can cause performance issues if the total number of DAGs is large, or if the code is connecting to an external system such as a database. For more on this, see the Scalability section below.
