...
> This guide summarizes some of the key practices and findings from our blog series with Updater about using dbt in Airflow. For more information, check out [Part 1](https://www.astronomer.io/blog/airflow-dbt-1), [Part 2](https://www.astronomer.io/blog/airflow-dbt-2), and [Part 3](https://www.astronomer.io/blog/airflow-dbt-3) of the series.
...
For this example we'll use the `BashOperator` because it lets us run some specific dbt commands.

In Airflow, a task that invokes the `BashOperator` simply executes a shell command. Because the primary dbt interface is the command line, the `BashOperator` is one of the best tools managing dbt. You can execute `dbt run` or `dbt test` directly in Airflow as you would with any other shell.
...
Using the `BashOperator` to run `dbt run` and `dbt test` is a working solution, but one without much visibility into what is going on. Failures are absolute and require the whole `dbt` group of models to be run again, which can be costly. The whole DAG is only two Tasks, though, which is great when you want to keep things simple.
...
Now that we know how to split dbt models into tasks, our next questions to tackle are:
...
- The parser takes the dbt project path containing the `dbt_project.yml` file, as well as the path to the `profiles.yml` file, as inputs. Note that this setup assumes that you have a single repo with that contains both your dbt project and Airflow code.
